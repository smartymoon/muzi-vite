<template>
  <div class="relative py-12 min-h-screen bg-white">
    ...
    <div class="p-4">
      ...
      <!-- 热门搜索 -->
      <div class="mt-2">
        <p class="text-sm font-bold p-2">热门搜索</p>
        <div class="flex flex-wrap items-center">
          <template v-for="(item,index) in hotList" :key="index">
            <button class="my-1.5 mr-3.5 py-1 px-3 bg-gray-200 text-gray-500 rounded text-xs" @click="onSearch(item)">{{item}}</button>
          </template>
        </div>
      </div>
    </div>
...
export default {
  setup() {
    ...
    const hotList = ref(['第一三共新露露','感冒药','更年期保健药','胃药','咽喉消炎','眼药水','大幸药品','小林制药','乐敦制药'])
