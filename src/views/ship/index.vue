    ...
    <main v-if="list.length > 0" class="space-y-3.5">
      ...
      <section class="bg-white p-4 rounded-b-2xl">
        <div class="w-full flex space-x-4">
          <van-image width="100" height="100" :src="data.img" lazy-load radius="7" class="flex-shrink-0 relative">
            <template v-slot:loading>
              <van-loading type="spinner" size="20" />
            </template>
            <div class="absolute bottom-0 h-4 inset-x-0 text-center text-xs text-white" style="background-color:rgba(0,0,0,0.6)">
              共{{ data.sum }}件商品
            </div>
          </van-image>
